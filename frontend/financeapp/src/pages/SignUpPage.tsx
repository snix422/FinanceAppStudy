...
const SignUpPage = () => {
    ...
    const onSubmit : SubmitHandler<SignUpTypeInputs> = async (formData:SignUpTypeInputs) => {
        ...
        try {
            await registerUser({email:formData.email,password:formData.password, name:formData.name,surname:formData.surname})
            reset();
        } catch (error:any) {
            dispatchError(error.response.data || "Wystąpił błąd podczas rejestracji.");
        }
    ...
    return (
        <main className="register-page">
            ...
            <form className="register-form" onSubmit={handleSubmit(onSubmit)}>
                ...
                <input className="input-field" type="password" {...register("password",validateOptions.password)}placeholder="Wpisz swoje hasło..." />
                {errors.password?.message ? <span className="error-message">{errors.password.message}</span>:null}
                <input className="input-field" type="password" {...register("confirmPassword",validateOptions.confirmPassword)} placeholder="Powtórz hasło" />
